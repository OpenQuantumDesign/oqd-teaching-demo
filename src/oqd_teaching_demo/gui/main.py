    ...
    with ui.dialog() as digital_dialog, ui.card():
        ...
        with ui.row().classes("fixed-center"):
            with ui.list():
                ui.button(
                    "Shor's Algorithm",
                    on_click=lambda: board.device.run(digital_shor()),
                ).props("border p-33")
                ui.button(
                    "Random Circuit",
                    on_click=lambda: board.device.run(digital_random()),
                ).props("border p-3")
...
def analog_card(board: Board):
    with ui.dialog() as analog_dialog, ui.card():
        with ui.row().classes("fixed-center"):
            with ui.list():
                ui.button(
                    "Nearest Neighbours Ising",
                    on_click=lambda: board.device.run(analog_ising()),
                )
                ui.button(
                    "All-to-All Interactions",
                    on_click=lambda: board.device.run(analog_all_to_all()),
                )
...
def main():
    ...
    with ui.column():
        ...
        ui.image(
            "https://github.com/OpenQuantumDesign/equilux/blob/9ed0c5380133e7d135121c44c3f4cdbcb8cf781b/docs/img/oqd-logo.png?raw=true"
        ).classes("w-32 h-32")
